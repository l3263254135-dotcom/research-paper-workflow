...
def main() -> int:
    ap = argparse.ArgumentParser(); ap.add_argument("results", type=Path); ap.add_argument("claims", type=Path); ap.add_argument("manuscript", nargs="+", type=Path); ap.add_argument("--json", action="store_true")
    args = ap.parse_args()
    try:
    ...
    out={"manuscripts":len(args.manuscript),"markers_checked":checked,"errors":errors,"status":"ok" if not errors else "failed"}
